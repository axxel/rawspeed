...
IsoMCanonCMT1Box::IsoMCanonCMT1Box(const AbstractIsoMBox& base)
  ...
  mRootIFD0 = parseTiff(data.getBuffer(data.getRemainSize()));
}
...
IsoMCanonCmp1Box::IsoMCanonCmp1Box(const AbstractIsoMBox& base)
  ...
  nBits = data.getByte();
  nPlanes = data.peekByte() >> 4;
  cfaLayout = data.getByte() & 0xF;
  encType = data.peekByte() >> 4;
  imageLevels = data.getByte() & 0xF;
  hasTileCols = data.peekByte() >> 7;
  hasTileRows = data.getByte() & 1;
...
IsoMCanonCmp1Box::operator bool() const {
  ...
  if (nPlanes == 1) {
    ...
    ThrowRDE("Unknown encoding parameters in CMP1 (%d, %d, %d, %d, %d, %d, %d)",
             nPlanes, f_width, f_height, tileWidth, tileHeight, cfaLayout,
             nBits);
...
IsoMCanonIad1Box::IsoMCanonIad1Box(const AbstractIsoMBox& base)
    : IsoMFullBox(base) {
  sensorWidth = data.getU16();
  sensorHeight = data.getU16();
  reserved1 = data.getU16();
  ind = data.getU16();
  reserved2 = data.getU16();
  reserved3 = data.getU16();

  if (2 == ind) { // ind is 2 for big images
    cropLeftOffset = data.getU16();
    cropTopOffset = data.getU16();
    cropRightOffset = data.getU16();
    cropBottomOffset = data.getU16();

    leftOpticalBlackLeftOffset = data.getU16();
    leftOpticalBlackTopOffset = data.getU16();
    leftOpticalBlackRightOffset = data.getU16();
    leftOpticalBlackBottomOffset = data.getU16();

    topOpticalBlackLeftOffset = data.getU16();
    topOpticalBlackTopOffset = data.getU16();
    topOpticalBlackRightOffset = data.getU16();
    topOpticalBlackBottomOffset = data.getU16();

    activeAreaLeftOffset = data.getU16();
    activeAreaTopOffset = data.getU16();
    activeAreaRightOffset = data.getU16();
    activeAreaBottomOffset = data.getU16();
  } else {
...
CanonTimedMetadata::CanonTimedMetadata::Record::Record(ByteStream* bs) {
  assert(bs->isInNativeByteOrder() == (Endianness::little == getHostEndianness()));
  auto origPos = bs->getPosition();
  recSize = bs->getU32();
  recType = bs->getU16();
  reserved1 = bs->getByte();
  reserved2 = bs->getByte();
  reserved3 = bs->getU16();
  reserved4 = bs->getU16();
...
CanonTimedMetadata::CanonTimedMetadata(const ByteStream* bs) : data(*bs) {
  ...
  data.setInNativeByteOrder(getHostEndianness() == little);
...
bool Cr3Decoder::isCodecSupported(const std::string& compressorVersion) const {
  ...
  return compressorVersion == "CanonCR3_001/00.11.00/00.00.00" // R8
         ...
         || compressorVersion ==
                "CanonCR3_001/01.09.00/00.00.00" // SX70 HS, G5 Mark II and G7
                                                 // Mark III
         || compressorVersion ==
...
void Cr3Decoder::decodeMetaDataInternal(const CameraMetaData* meta) {
  ...
  //                           8); // skip TIFF header
  //TODO axxel
  TiffRootIFD IFD_ctmd_rec8(DataBuffer(rec8), 8); // skip TIFF header
...
TiffIFD* Cr3Decoder::getRootIFD() { return canonBox->CMT2()->mRootIFD0.get(); }
