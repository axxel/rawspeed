...
IsoMParser::IsoMParser(FileMap* inputData) : RawParser(inputData) {}

void IsoMParser::parseData() {
  ByteStream bs(DataBuffer(*mInput, getHostEndianness() == big));
...
RawDecoder* IsoMParser::getDecoder(const CameraMetaData* meta) {
  if (!rootBox)
    parseData();

  if (Cr3Decoder::isAppropriateDecoder(*rootBox))
    return new Cr3Decoder(std::move(rootBox), mInput);
